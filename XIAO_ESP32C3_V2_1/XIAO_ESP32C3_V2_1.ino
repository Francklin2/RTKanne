...
const unsigned long RTCM_WRITE_INTERVAL = 1;
const int RTCM_BYTES_PER_WRITE = 80;
...
const unsigned long RECONNECT_DELAY = 500;

String nmeaBuffer;

// Fonctions buffer RTCM (inchangées)
...
// Callbacks BLE — signature simple sans esp_ble_gatts_cb_param_t
class MyServerCallbacks : public BLEServerCallbacks {
  void onConnect(BLEServer* pServer) {
    deviceConnected = true;
    wasConnected = true;
    // iOS négocie automatiquement les paramètres de connexion BLE
  };

  void onDisconnect(BLEServer* pServer) {
    deviceConnected = false;
    disconnectionTime = millis();
    // Le watchdog dans loop() gère le re-advertising avec délai
  }
};
...
void loop() {
  // Watchdog reconnexion BLE avec délai
  if (!deviceConnected && wasConnected) {
    if ((millis() - disconnectionTime) >= RECONNECT_DELAY) {
      wasConnected = false;
      BLEDevice::startAdvertising();
    }
  }

  // Gestion RTCM paramètres fixes
  if ((millis() - lastRtcmWrite) >= RTCM_WRITE_INTERVAL) {
    int bufferLevel = getBufferLevel();
    if (bufferLevel > 0) {
      uint8_t rtcmData[120];
